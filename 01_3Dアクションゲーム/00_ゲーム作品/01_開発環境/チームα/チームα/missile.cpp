...
#define MISSILE_ATTACK			(100)						// ミサイルの攻撃力
#define MISSILE_GRAVITY			(0.05f)						// 重力
#define MISSILE_COLLISION_SIZE	(50.0f)						// 当たり判定のサイズ
#define MISSILE_FOLLOW_DISTANCE	(400.0f)					// 追従の距離
	...
	const MISSILE_USER user, float fSpeed)
{
	...
	{
		// 初期化処理
		pMissile->m_user = user;		// ユーザーの種類
		pMissile->Init(pos, size);		// 初期化情報
		pMissile->m_fSpeed = fSpeed;	// 速度
	}
	else
	{
		return NULL;
...
bool CMissile::Collision(void)
{
	if (m_pTargetPL != NULL)
	{
		...
		// 当たり判定
		if (targetPos.x + PLAYER_COLLISION_X / 2 >= m_pos.x - MISSILE_COLLISION_SIZE &&
			targetPos.x - PLAYER_COLLISION_X / 2 <= m_pos.x + MISSILE_COLLISION_SIZE &&
			targetPos.y + PLAYER_COLLISION_Y >= m_pos.y - MISSILE_COLLISION_SIZE &&
			targetPos.y - 0.0f <= m_pos.y + MISSILE_COLLISION_SIZE &&
			targetPos.z + PLAYER_COLLISION_Z / 2 >= m_pos.z - MISSILE_COLLISION_SIZE &&
			targetPos.z - PLAYER_COLLISION_Z / 2 <= m_pos.z + MISSILE_COLLISION_SIZE)
		{
...
bool CMissile::Follow(void)
{
	...
	float fPposx = pPlayer->GetPos().x, fPposz = pPlayer->GetPos().z;		// 相手の座標
	float fBposx = m_pos.x, fBposz = m_pos.z;							// 弾の座標
	float fAngle;														// 角度

	fAngle = atan2f((fBposx - fPposx), (fBposz - fPposz));				// 角度を決める
	...
	float fDistance = sqrtf(
		...
		// 一定の距離に行くまでは追従
		if (fDistance > MISSILE_FOLLOW_DISTANCE)
		{
			return true;
		}
		else
